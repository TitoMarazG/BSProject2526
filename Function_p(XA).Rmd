...
Qui creo a parte una funzione che calcola la multivariate gamma function, la costante di normalizzazione c(a,U) e finalmente poi la p(XA). Siccome i numeri sono giganti, calcolo tutto in log e poi calcolo l'esponenziale.
Nota per me stesso: in futuro sarebbe carino dare in pasto solo la prior anziché a ed U separati.

###p(XA)
```{r p(XA)}
p_XA <- function(X, a, U, A){
  return(exp(log.p_XA(X, a, U, A)))
}
```

###log(p(XA))
```{r}
log.p_XA <- function(X, a, U, A){
  S <- t(X)%*%X
...
Utilizzo la stima della media di Ω:
E[Ω] ≈ a*U
Ω.cappuccio = ∑^-1 (stima dai dati)
=> U = E[Ω]/a = ∑^(-1)/a
```{r}
a = 100   # unica cosa che decido io
Sigma = cov(X)
Ω.cappuccio = solve(Sigma)
U = Ω.cappuccio/a
log.p_XA(X, a, U, A)
```
```{r}
...
for (i in 1:(100-q+1)) {
  log_probs[i] <- log.p_XA(X, a_vals[i], lista_U[[i]], A)
}
```
```{r}
