...
res_bayes <- readRDS("C:/Users/arian/Documents/GitHub/BSProject2526/test/Riboflavin_Bayes_q1000.rds")
...
if(length(neigh)>0){
  ...
  P <- matrix(runif(length(nodes_in_sub)^2), 
              nrow = length(nodes_in_sub), 
              ncol = length(nodes_in_sub),
              dimnames = list(nodes_in_sub, nodes_in_sub))
